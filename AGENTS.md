...
- **Repository:** nogic1008/ha_aircloudhome

**Key directories:**

- `custom_components/aircloudhome/` - Main integration code
...
- Integration logs: `custom_components.aircloudhome: debug` in `config/configuration.yaml`
...
- Use the domain `aircloudhome` for all DOMAIN references
- Prefix all integration-specific classes with `AirCloudHome`
- Use "Shirokuma AC (aircloudhome) Integration" as the display title
...
- Inherit from platform base + `AirCloudHomeEntity`
...
- `tests/` mirrors `custom_components/aircloudhome/` structure
