...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `AirCloudHome`
...
- Inherit from both platform entity and `AirCloudHomeEntity` (order matters)
...
from custom_components.aircloudhome.sensor import async_setup_entry
...
**Config entry data:** `entry_data: AirCloudHomeData = hass.data[DOMAIN][entry.entry_id]`
