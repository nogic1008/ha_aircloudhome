...
**Directory:** `custom_components/aircloudhome/[platform]/`
...
```python
"""[Platform] platform for Shirokuma AC (aircloudhome) Integration."""
...
from .entity import AirCloudHomeEntity
from .[entity_file] import AirCloudHome[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: AirCloudHomeDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            AirCloudHome[EntityName](coordinator, entry),
...
```python
"""[Entity description] for Shirokuma AC (aircloudhome) Integration."""
...
Add platform to `custom_components/aircloudhome/manifest.json`:
...
- [ ] Entity class inherits from both `AirCloudHomeEntity` and platform class
...
- **Base entity:** `AirCloudHomeEntity` in `entity/base.py`
- **Coordinator:** `AirCloudHomeDataUpdateCoordinator`
