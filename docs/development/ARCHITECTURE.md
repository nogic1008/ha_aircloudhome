...
This document describes the technical architecture of the Shirokuma AC (aircloudhome) Integration custom component for Home Assistant.

## Directory Structure

```text
custom_components/aircloudhome/
...
│   ├── __init__.py          # Exports AirCloudHomeDataUpdateCoordinator
...
│   ├── __init__.py          # Exports AirCloudHomeEntity
...
- `base.py` - Main coordinator class (`AirCloudHomeDataUpdateCoordinator`)
...
**Key class:** `AirCloudHomeDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `AirCloudHomeApiClient`
...
- `AirCloudHomeConfigFlowHandler` (main flow)
- `AirCloudHomeOptionsFlow` (options)
...
**Key class:** `AirCloudHomeEntity` (in `entity/base.py`)
...
2. `AirCloudHomeEntity` for common functionality
...
1. Create directory: `custom_components/aircloudhome/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `AirCloudHomeEntity`
